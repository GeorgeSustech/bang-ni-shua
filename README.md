...
.\.venv\Scripts\python.exe cli.py run 英语 --sound        # 本次不静音（默认静音播放）
...
`--speed` 指定播放倍速（`0.5`–`2`，`1` 为原速），**默认 2 倍速**，并存进 `settings.json` 供下次沿用；双击向导里也能改，直接回车就保持上次的值。程序走的是**雨课堂自带的倍速控件**（0.5X / 1.00X / 1.25X / 1.50X / 2.00X），所以平台按它自己的档位记录播放，不会跳转、也不会修改任何完成状态字段；如果请求的倍速不在平台的档位里，会取最接近的一档并提示。

默认**静音播放**（把播放器音量锁到 0），需要听声音时加 `--sound`。

命令行版的退出码：`0` 已开放视频全部完成 · `1` 仍有未完成项 · `2` 手动停止 · `3` 受阻或出错。

### 3.3 运行行为

程序默认按 2 倍速、静音依次播放视频，遇到无法识别的弹窗、弹题、测验或页面卡住，会记录原因、发送通知并尝试下一节。视频播放结束但平台未记入进度时，程序会刷新页面并在一小时后复查；复查期间继续播放其他视频。运行期间每 6 小时发送一次状态，队列结束后发送退出结果。

雨课堂的播放器带防挂机逻辑：窗口失焦或被隐藏时它会主动暂停播放。程序检测到暂停后会在几秒内自动继续，所以专用 Chrome 窗口放在后台也能接着播。

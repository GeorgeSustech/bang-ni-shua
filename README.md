...
`--speed` 指定播放倍速（`0.5`–`4`，`1` 为原速），并按 `settings.json` 记住，下次不带参数也沿用；双击向导里也会问一次倍速，直接回车即保持上次的值。倍速只在播放器里设置 `playbackRate`，不会跳转或修改任何完成状态字段。需要注意：**平台可能不把加速后的时长计入进度**，这类视频会走"一小时复查→未记入"的流程并在结果里列出，所以是否提速由你自己权衡。

命令行版的退出码：`0` 已开放视频全部完成 · `1` 仍有未完成项 · `2` 手动停止 · `3` 受阻或出错。

### 3.3 运行行为

程序默认按正常倍速依次播放视频，遇到无法识别的弹窗、弹题、测验或页面卡住，会记录原因、发送通知并尝试下一节。视频播放结束但平台未记入进度时，程序会刷新页面并在一小时后复查；复查期间继续播放其他视频。运行期间每 6 小时发送一次状态，队列结束后发送退出结果。
